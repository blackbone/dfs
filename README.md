# dfs
distributed file system (coded by AI)

# same as they use in google
...
ence to Figure 1. First, using the fixed chunksize, the client
...
as a single-chunkfile and then started on hundreds of ma-
...
open of the file, which purges from the cache all chunkin-
...
last chunkof the file Then, it sends its request to the pri-
...
traffic, especially reads, for a chunkcan exploit the aggre-
...
below-average diskspace utilization. Over time this will
...
regular garbage collection at both the file and chunklevels.
...
Chunkreplicas may become stale if a chunkserver fails
...
its log record has been flushed to disklocally and on all
...
other machines. If a blockdoes not match the recorded
...
networkinterface gets saturated, whichever applies. The
...
ited by the networkbandwidth of the chunkservers that
store the last chunkof the file, independent of the num-
ber of clients. It starts at 6.0 MB/s for one client and drops
to 4.8 MB/s for 16 clients, mostly due to congestion and
variances in networktransfer rates seen by different clients.
...
includes all chunkreplicas. Virtually all files are replicated
...
5
10
Append rate (MB/s) Network limit
...
sources efficiently. Cluster B can support peakread rates of
...
master. Most requests askfor chunklocations ( FindLo-
...
tually, we found that this single lockblocked the primary
